...
public class ZKClientWatcher {
    ...
    static {
        ...
        try {
            ...
            zk = new ZooKeeper(zkHost, 3000,watcher);
        } catch (IOException e) {
        }
    }
...
//        ZKClient.delete();
    ...
    public static void createNode() throws KeeperException, InterruptedException {
        ...
        System.out.println(new String(zk.getData("/zkStudy", watcher, null)));
    ...
    public static void getData() throws KeeperException, InterruptedException {
        Stat tokenNode = zk.exists("/zkStudy", watcher);

        String str = new String(zk.getData("/zkStudy", watcher, null));
        System.out.println("节点 ： " + str);

        // 取出子目录节点列表
        List<String> children = zk.getChildren("/zkStudy", watcher);
